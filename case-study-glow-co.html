    ...
    <section className="section about-diff-section" aria-label="What makes us different">
      <div className="container">
        ...
        <div className="about-diff-grid">
          {items.map((it, i) => (
            <a href={"why-ignited-spa.html#diff-0" + (i + 1)} className={"about-diff-card reveal in d" + (i % 3)} key={i} style={{textDecoration:'none',color:'inherit',display:'block'}}>
              <div className="adc-num serif-i">{it.n}</div>
              <h3>{it.t}{it.em && <span className="adc-badge">Unique</span>}</h3>
              <p>{it.d}</p>
              <span style={{display:'inline-flex',alignItems:'center',gap:'6px',marginTop:'16px',fontSize:'13px',fontWeight:'500',color:'var(--navy-2)'}}>
                Learn more
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M13 5l7 7-7 7"/></svg>
              </span>
            </a>
          ))}
        </div>
